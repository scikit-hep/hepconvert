...
def HZZ_test():
    ...
    od.root_to_parquet(input_file=data_path("uproot-HZZ.root"), output_file="test.parquet", step_size="100 MB")
...
def Zmumu_test():
    ...
    original = f['events'].arrays()
    od.root_to_parquet(input_file=data_path("uproot-Zmumu.root"), output_file="test1.parquet", step_size="100 MB")
...
def break_trees():
    with pytest.raises(AttributeError):
        od.root_to_parquet(input_file=data_path("uproot-hepdata-example.root"), output_file="test2.parquet", step_size="100 MB")
    # with pytest.raises(AttributeError):
