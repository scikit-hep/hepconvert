...
def test_copy(tmp_path):
    ...
    for key in hepconvert_file["events"].keys():
        if key.startswith("Jet_"):
            ...
            assert ak.all(
                hepconvert_file["events"][key].array() == file["events"][key].array()[file["events"]['Jet_Px'].array()>=10]
            )

def test_trigger(tmp_path):
    file = uproot.open(skhep_testdata.data_path("nanoAOD_2015_CMS_Open_Data_ttbar.root"))
    ...
    for key in hepconvert_file["Events"].keys():
        if key.startswith("Jet_"):
            assert key in file["Events"].keys()
            assert ak.all(
                hepconvert_file["Events"][key].array() == file["Events"][key].array()[file["Events"]['HLT_QuadPFJet_DoubleBTagCSV_VBF_Mqq200'].array()]
            )
...
def test_merge_cut(tmp_path):
    ...
    for key in hepconvert_file['events'].keys():
        if key.startswith("Jet_"):
            ...
            assert ak.all(hepconvert_file['events'][key].array()[:2421] == file[key].array()[file['Jet_Px'].array()>=10])
...
def test_merge_trigger(tmp_path):
    ...
    for key in hepconvert_file['events'].keys():
        if key.startswith("Muon_"):
            assert key in file.keys()
            assert ak.all(hepconvert_file['events'][key].array()[:2421] == file[key].array()[file['Muon_Iso'].array()>3])
